...
按2026-09-16的研究者决定，发病到MRI的天数不再提取、不作为调整因素或入组条件。研究将本次住院MRI产出视为基线结构表型；由于不使用扫描日期，无法由本分析核定扫描与早期复发的先后。该时间信息边界在报告中说明。
...
**2026-09-17数值求解维护（在一年结果查看后记录）：**模型公式和非惩罚部分似然目标保持不变。原Newton求解失败时，对设计矩阵进行中心化和尺度变换，以BFGS获得初值后再用Newton求解，系数和协方差转换回原尺度。接受条件包括系数/协方差有限、正方差、标准化信息矩阵正定且条件数不超过1e12，以及标准化得分最大绝对值/事件数不超过1e-6。这些是数值检查阈值，不是统计充分性判定。保留失败状态，不自动删变量或引入惩罚。bootstrap原90%成功门槛不变，新增每次失败所属阶段记录。该维护不解决比例风险假设或稀疏事件造成的估计不精确，也不改动已有结果文件。
...
`白名单SAS→CSV与元数据 → ID精确连接/WMH与ICV体积可用性检查 → 各假设病例清单 → 固定协变量集合 → H1结构 → H2复发 → H3复测更新 → H4功能 → 假设判定总表`。

按研究者2026-09-16修订，所有分析不以影像人工质控为入组条件。主队列按WMH和真实ICV的数值可用性纳入；T1及急性损伤扩展分别按相应体积可用性纳入。既有审核状态不改写、不参与筛选；文件、单位、体积有限性及ID检查仍执行。
