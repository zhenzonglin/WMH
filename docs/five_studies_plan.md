...
| 06 脑肾微血管损伤 | 恢复期功能和WMH相近时持续白蛋白尿是否提示远期依赖 | 多分类模型依赖方程中持续升高与两次均低的对比 |
...
主要临床对比是WMH第25、50、75百分位处，SBP 130相对140 mmHg的HR及95%区间。120和150相对140作为补充展示。对比使用完整协方差。各WMH展示点周围的分位区间至少有20名受试者且涵盖两个比较血压值时才显示；否则报告超出观测支持，不换参考点寻找显著结果。这是外推检查，不是临床阈值。
...
调整集合为研究04的主要集合，另固定加入HDL-C。CEC的解释为HDL-C相近条件下的结构差异，不因HDL-C的P值不显著而删除它。异常CEC值不依据样本分位数自动截尾；非法数值和单位冲突由审计报告。
...
主要交付为CEC每1 SD对应的灰质体积差异、95%区间、主要P值，以及分开标记的次要结果表。若置信区间接近零且较窄，则该人群下的结构差异可能有限；若较宽，则首先说明精度不足。

<!-- PAGE -->

## 06 脑肾微血管损伤

CNSR-III已发表基线和3个月白蛋白尿与一年预后的分析，持续白蛋白尿与不良结局相关。[7] 本研究的新增问题是在基线WMH和3个月功能相近时，两次白蛋白尿信息是否仍与五年依赖相关。因此研究重点是额外的条件预后信息，而不是再次证明白蛋白尿与一年结局有关。
...
主要假设为：在WMH、恢复期mRS、SBP和肾滤过功能等背景相近时，持续升高相对两次均低与较高的五年依赖/独立概率比相关。主要模型为三状态多分类logistic，唯一主要检验为依赖方程中持续升高类别的系数。

调整C、BMI、教育、3个月胱抑素C、3个月SBP、出院ACEI/ARB、3个月mRS、ICV及连续WMH。没有Hcy要求；不使用字典中实际含义为表皮生长因子受体的eGFR字段。降压子项明确未用药者可依据父项“未使用”归零；相互矛盾的记录标记而非任意覆盖。

次要分析为：基线UACR与基线WMH，使用基线C、BMI、教育、CYSC、入院SBP和ICV；连续log(1＋M03_UACR)与五年功能状态并调整基线log(1＋UACR)；同样本临床＋WMH与增加三项白蛋白尿类别的模型，用联合Wald检验评估依赖方程增量关联；加入NIHSS、TOAST和急性梗死体积并与同一子样本原模型比较。

主要版本不增加UACR×WMH交互。基线结构模型不调整未来3个月指标。报告完整病例和结局可观测概率加权敏感性分析。两次UACR不插补，因此缺少任一次测量者不构造“持续”分组。
...
分类参考值按字典顺序在实际观察类别中固定。主队列某合法类别完全未出现时，该类别没有可估计系数；这不是删除该变量。后续子队列或重编码造成固定矩阵奇异时明确失败，不临时合并类别。
...
本次版本固定五项问题及主要检验，代码合同标识为imaging_five_studies_20260917_v1。细化执行规则包括：血压同值固定取左侧；功能验证补充基线卒中严重程度；观测支持限制仅用于图表；急性梗死为结局时移除相同调整项；权重估计采用堆叠sandwich；无估计项保留预设多重比较家庭大小。
