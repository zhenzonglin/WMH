...
调整年龄、性别、吸烟、饮酒、高血压、糖尿病、既往卒中、BMI、基线胱抑素C、入院SBP、冠心病、TOAST、卒中前及出院降压用药、3个月mRS和ICV。后续处方不回填此前暴露。本版按研究者决定取消IMG_ICAS调整项，所有血压敏感性分析沿用这一调整集合。
...
其他纳入方案的字段继续按多SAS来源审计提取。本版不提取或分析IMG_ICAS和Apo_AI，不以两字段的有无限制入组。字段是否存在和是否实际有测量值分别计数，新增数据不改变单位、编码或精确ID连接规则。
...
固定次要分析包括CEC与WMH、CEC与五年三状态功能结局、同样本加入GM119后的条件关联。长期功能模型另补充卒中前mRS、NIHSS和TOAST，并有结局可观测概率加权敏感性分析。CEC三节点样条为函数形式敏感性分析。

每个次要分析只要求对应指标，不因为某项次要检测缺失而缩小主要队列。本版取消Apo_AI扩展模型；HDL-C主模型及不调整HDL-C的配对比较保持不变。主版不扫描119个脑区。不能以“CEC显著而HDL-C不显著”宣布CEC预测更好；本轮没有开展外部验证或临床增益评价。

图表并列M0、M1的效应及95%区间。CEC负值、非有限值和非空不可解析值仅从CEC研究排除，记录原因，不插补、不再确认；零值允许，普通缺失单列，不按分位数截尾。原始数据不改，单位冲突仍报告。
...
| CEC和常规血脂 | CEC、BSL_HDL、BSL_LDL、BSL_TG |
...
工作站只需执行wmh-study start。命令从最新输入重新准备四个队列，自动保存来源、覆盖交集、排除、结局、缺失和冲突，随后连续拟合并生成报告，无需另输审计或启动命令。真实输入错误只阻断对应研究，其余研究继续，最后汇总状态。audit和diagnose保留为可选诊断入口；start --study cec等可单独重跑。
...
第五版标识为imaging_four_studies_20260918_v5。根据研究者2026年9月18日决定，明确排除非法CEC测量，并以start一键完成准备、分析和报告。第四版取消IMG_ICAS和Apo_AI；第三版移除神经酰胺及视觉重度WMH分析，并加入冠心病跳答规则。原编号01、02、05、06和四项主要检验保持不变，报告p_holm_four；研究内次要分析BH-FDR家庭不变。修订不依据效应或P值。旧结果保留并标为PREVIOUS_VERSION，不与本版混合；本次修订不称为原始预注册方案。

流程自动检查每项研究的字段来源、单位、临床影像交集、五年三状态人数或血压风险集事件数。任何关于有效样本、精度或最终发现的陈述，必须以工作站新运行的审计与结果为依据。
