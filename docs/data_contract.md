...
采用日精度。`entry=max(采血日−发病日, MRI距发病日)`，风险区间为(entry, exit]。同日不能确定测量先后的事件单列排除。PHReg传入nextafter(entry,+∞)仅实现开放左端点，不创造临床事件时间。
...
H1使用基线影像/代谢合格病例，不要求结局完整。H2在测量完成后进入风险集。H3使用实际M3采血日及同期HCY/B12/B9/CYSC，保留基线Hcy和WMH信息。H4要求mRS及合格急性损伤体积，T1另建子样本。H_STROKE进入v2核心调整集合。源字段白名单仍为39项，未增加临床数据字段。
