...
第三页已确认数据批次一致，但死亡模型仍存在数值问题时，先执行独立拟合检查：

```bash
git pull --ff-only
wmh-hcy check-fit --hypothesis H3
```

此命令使用现有prepared队列，按当前配置重新完成插补（生产配置50份）并分别检查缺血和死亡模型。它需要计算时间，但不做bootstrap、不重读SAS和影像、不改写正式结果指针，也不运行H1/H4。输出只打印事件计数、成功份数、备用求解恢复份数及数值诊断；截取最终摘要即可。新日志保存在`outputs/real/diagnostics/fit_check_<时间>/`，不包含患者行。H3通过后，可将参数改为H2检查主队列。

备用求解仅在原Newton失败或未通过数值检查时启动：设计矩阵中心化及标准差缩放，BFGS取得初值，再用Newton收敛到同一非惩罚部分似然解，最后还原系数和协方差。没有删除协变量、合并分类或改变交互项。仍失败者保留FAIL；不把数值收敛当作事件数充足或比例风险假设成立。此更新不自动改写已经保存的研究结果。
